...
1.  **You:** Define task, context, I/O needs for `promptbuilder`.
2.  **`promptbuilder`:** Generates a META_PROMPT.
...
5.  **You:** Use the generated TASK_PERFORMING_PROMPT for your original task. Replace placeholders if necessarry.
...
```
*(Output: META_PROMPT printed, instructing the LLM to create a TASK_PERFORMING_PROMPT that includes `{{INPUT}}` / `{{OUTPUT}}` placeholders or sections and format instructions.)*

**4. Using Files for Long Instructions or Examples:**
Provide detailed instructions or examples from files instead of directly on the command line.

```bash
# Create instruction files first (e.g., meta_guide.txt, example_in.txt, example_out.txt)
...
**5. Saving to File and Copying to Clipboard:**
...
```
*(Output: Follow the prompts to enter task description, context paths, instructions, etc.)*
